...
class Charger(models.Model):
    ...
    status = models.CharField(
        "Status", max_length=20, choices=STATUS_CHOICES, default="idle"
    )
...
class ChargingRecord(models.Model):
    ...
    charger = models.ForeignKey(
        Charger, on_delete=models.CASCADE, related_name="records"
    )
    user = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, related_name="charging_sessions"
    )
    start_time = models.DateTimeField("Start Time")
    end_time = models.DateTimeField("End Time", null=True, blank=True)
    duration = models.IntegerField("Duration (minutes)", null=True, blank=True)
    electricity = models.DecimalField(
        "Energy (kWh)", max_digits=5, decimal_places=2, null=True, blank=True
    )
    fee = models.DecimalField(
        "Total Fee (€)", max_digits=6, decimal_places=2, null=True, blank=True
    )
    pay_status = models.CharField(
        "Payment Status", max_length=10, choices=PAY_STATUS, default="unpaid"
    )
    status = models.CharField(
        "Status", max_length=15, choices=STATUS, default="not started"
    )
    transaction_id = models.CharField(
        "Transaction ID", max_length=100, blank=True, null=True
    )
...
class MaintenanceRecord(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    charger = models.ForeignKey(
        Charger, on_delete=models.CASCADE, related_name="maintenances"
    )
    worker = models.ForeignKey(
        User,
        on_delete=models.SET_NULL,
        null=True,
        limit_choices_to={"is_maintenance": True},
    )
    maintenance_type = models.CharField(
        "Type", max_length=20, choices=MAINTENANCE_TYPES
    )
    content = models.TextField("Details")
    maintenance_time = models.DateTimeField("Maintenance Time")
    duration = models.IntegerField(
        "Duration (minutes)", help_text="Time spent on maintenance"
    )
