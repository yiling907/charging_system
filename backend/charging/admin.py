...
import uuid
from .models import Station, Charger, ChargingRecord, MaintenanceRecord, User
...
class StationAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Basic Information", {"fields": ("name", "address", "latitude", "longitude")}),
        (
            "Operational Status",
...
class ChargerAdmin(admin.ModelAdmin):
    ...
    def status_badge(self, obj):
        ...
        return format_html(
            ...
            color,
            dict(Charger.STATUS_CHOICES)[obj.status],
        )
...
class ChargingRecordAdmin(admin.ModelAdmin):
    ...
    def pay_status_badge(self, obj):
        ...
        color_map = {"unpaid": "danger", "paid": "success", "refunded": "secondary"}
        color = color_map.get(obj.pay_status, "secondary")
        return format_html(
            '<span class="badge bg-{}">{}</span>',
            color,
            dict(ChargingRecord.PAY_STATUS)[obj.pay_status],
        )
    ...
    def mark_as_paid(self, request, queryset):
        ...
        updated = queryset.filter(pay_status="unpaid").update(
            pay_status="paid", transaction_id=f"ADMIN-{uuid.uuid4().hex[:8]}"
        )
        self.message_user(
            request, f"Successfully updated {updated} records to paid status"
        )
