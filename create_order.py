...
def handler(event, context):
    ...
    payment_token = request_body.get('paymentToken')
...
def update_payment_status(record_id):
    headers = {'Content-Type': 'application/json'}
    response = requests.post(UPDATE_PAYMENT_STATUS_API + record_id + '/set_paid/', headers=headers)
...
def update_charger_status(charger_id):
    headers = {'Content-Type': 'application/json'}
    response = requests.post(UPDATE_CHARGER_STATUS_API + charger_id + '/set_inactive/', headers=headers)
    response.raise_for_status()
    return response.json()
...
def create_scheduled_task(charger_id, timeout_minutes):
    ...
    cloudwatch_events.put_targets(
        ...
        Targets=[{
            ...
            'Input': json.dumps({'charger_id': charger_id, 'target_status': 'idle'})
        }]
    )
